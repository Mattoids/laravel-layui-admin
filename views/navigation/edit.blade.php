<div class="layui-card-body">
    <form class="layui-form" method="post" action="{{ route("admin.navigation.update", ['navigation' => $navigation->id]) }}" id="layer-form">
        @csrf
            ...
            <div class="layui-input-block">
                <select name="parent_id" lay-verify="required">
                    ...
                    @foreach($navigation_list as $val)
                        <option value="{{ $val->id }}" @if($val->id == $navigation->parent_id) selected @endif>{{ $val->name }} [{{ $val->type }}]</option>
                    @endforeach
                </select>            </div>
        </div>
        <div class="layui-form-item">
